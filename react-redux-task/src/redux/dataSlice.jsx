...
const dataSlice = createSlice({
  ...
  reducers: {
    updateInCart: (state, action) => {
      ...
      return state.map((item) => item.id === id ? { ...item, inCart } : item);
    },
    resetData: (state) => {
      return state.map((item) => ({ ...item, inCart: false, defaultValue: 1 }));
    },
  },
});

export const { updateInCart, resetData } = dataSlice.actions;
